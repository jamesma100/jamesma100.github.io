...
To alleviate this bottleneck, CPU manufacturers have added layers of cache between the CPU and main memory, usually L1, L2 and L3, but even L4 in rare cases.
...
For one, caches, unlike CPU registers, have low granularity - usually 64 bytes for a single cache line.
...
First, we allocate a struct with two variables.
Unless you're using some esoteric allocator, we can expect them to reside on the same cache line.
Then, we can run some threads that updates these variables 3 billion times.
```
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>

long NUM_ITER = 3l * 1000l * 1000l * 1000l;
...
```
Here is the main method that creates two threads that run the above operation, with the second thread commented out initially:
```
int main() {
  pthread_t ta;
  // pthread_t tb;
...
Running the above code launches one thread, which takes around:
```
...
sys	0m0.002s
```
but if we uncomment the second thread, our `real` time goes up to ~18s.
Since thread 1 updates `a` and thread 2 updates `b`, we should expect the time to remain the same, as we are utilizing two cores in parallel.
But here we're seeing false sharing in action!
